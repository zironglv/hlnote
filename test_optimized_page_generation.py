...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('optimized_debug_page_generation.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

async def generate_optimized_test_report():
    """生成优化版的测试报告"""
    try:
        logger.info("开始生成优化版测试报告...")
        ...
        output_dir = os.path.join('test_reports', f'optimized_playwright_debug_{timestamp}')
        ...
        # 保存日报版本
        daily_report_path = os.path.join(output_dir, 'optimized_daily_report.html')
        with open(daily_report_path, 'w', encoding='utf-8') as f:
            f.write(daily_report_html)
        
        logger.info(f"优化版完整报告已保存到: {os.path.join(output_dir, 'optimized_index.html')}")
        logger.info(f"优化版日报已保存到: {daily_report_path}")
        logger.info(f"图表文件: {chart_path}")
        
        return os.path.join(output_dir, 'optimized_index.html'), daily_report_path
        
    except Exception as e:
        logger.error(f"生成优化版测试报告失败: {str(e)}", exc_info=True)
        return None, None

async def test_optimized_page_with_playwright(html_path, report_type="full", headless=True):
    """使用Playwright在无头模式下测试优化后的页面"""
    ...
    async with async_playwright() as p:
        ...
        # 截图以供检查
        screenshot_path = html_path.replace('.html', f'_{report_type}_optimized_screenshot.png')
        await page.screenshot(path=screenshot_path, full_page=True)
        logger.info(f"优化版页面截图已保存到: {screenshot_path}")
        ...
        try:
            ...
            if stats_grid:
                ...
                logger.info(f"✅ 找到 {len(stats)} 个指标卡片")
                
                # 输出每个指标的文本内容
                for i, stat in enumerate(stats):
                    value = await stat.query_selector(".stat-value")
                    label = await stat.query_selector(".stat-label")
                    if value and label:
                        value_text = await value.inner_text()
                        label_text = await label.inner_text()
                        logger.info(f"  指标 {i+1}: {label_text} = {value_text}")
            else:
                logger.warning("⚠️ 未找到统计网格")
            
            # 检查投资建议卡片
            advice_card = await page.query_selector(".advice-card")
            if advice_card:
                logger.info("✅ 找到投资建议卡片")
                
                # 检查建议动作
                advice_action = await page.query_selector(".advice-action")
                if advice_action:
                    action_text = await advice_action.inner_text()
                    logger.info(f"  投资建议: {action_text}")
            else:
                logger.warning("⚠️ 未找到投资建议卡片")
            
            # 检查趋势分析部分
            trend_analysis = await page.query_selector(".analysis-section")
            if trend_analysis:
                logger.info("✅ 找到分析部分")
            else:
                logger.warning("⚠️ 未找到分析部分")
...
async def main():
    """主函数 - 生成优化版报告并用Playwright测试"""
    logger.info("开始Playwright优化版页面生成调试...")
    
    # 生成优化版测试报告
    full_report_path, daily_report_path = await generate_optimized_test_report()
    
    if full_report_path:
        logger.info("=== 测试优化版完整报告页面 ===")
        success_full = await test_optimized_page_with_playwright(full_report_path, "full", headless=True)
        
        logger.info("=== 测试优化版日报页面 ===")
        success_daily = await test_optimized_page_with_playwright(daily_report_path, "daily", headless=True)
        
        if success_full and success_daily:
            logger.info("✅ 优化版页面生成和显示测试成功完成")
            print(f"\n📄 生成的优化版报告文件:")
            print(f"   优化版完整报告: {full_report_path}")
            print(f"   优化版日报: {daily_report_path}")
        else:
            logger.error("❌ 优化版页面测试未完全成功")
    else:
        logger.error("❌ 无法生成优化版测试报告")
